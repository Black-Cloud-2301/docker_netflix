import React from 'react';
import { SelectedState } from '.';

type Props = {
	setNextPage: (val: number) => void;
	selected: SelectedState;
};

const SixthBody: React.FC<Props> = ({ setNextPage, selected }) => {
	let money: string = '';
	switch (selected) {
		case 'mobile':
			money = '70000';
			break;
		case 'basic':
			money = '180000';
			break;
		case 'standard':
			money = '220000';
			break;
		case 'premium':
			money = '260000';
			break;
		default:
			break;
	}

	return (
		<div className='max-w-xl mx-auto p-8 sm:px-12'>
			<p className='text-sm'>
				STEP <b>3</b> OF <b>3</b>
			</p>
			<h3 className='text-3xl sm:text-4xl font-bold'>
				Set up your credit or debit card
			</h3>
			<div className='flex gap-2 my-4'>
				<svg
					xmlns='http://www.w3.org/2000/svg'
					viewBox='0 0 160 100'
					className='h-7'
				>
					<switch>
						<g>
							<path
								d='M8 99c-3.8 0-7-3.2-7-7V8c0-3.8 3.2-7 7-7h144c3.8 0 7 3.2 7 7v84c0 3.8-3.2 7-7 7H8z'
								fillRule='evenodd'
								clipRule='evenodd'
								fill='#fff'
							/>
							<path
								d='M152 2c3.3 0 6 2.7 6 6v84c0 3.3-2.7 6-6 6H8c-3.3 0-6-2.7-6-6V8c0-3.3 2.7-6 6-6h144zm0-2H8C3.6 0 0 3.6 0 8v84c0 4.4 3.6 8 8 8h144c4.4 0 8-3.6 8-8V8c0-4.4-3.6-8-8-8z'
								fill='#e6e6e6'
							/>
							<linearGradient
								id='a'
								gradientUnits='userSpaceOnUse'
								x1='33.507'
								y1='59.005'
								x2='133.468'
								y2='39.797'
							>
								<stop offset='0' stopColor='#22275d' />
								<stop offset='1' stopColor='#24479b' />
							</linearGradient>
							<path
								d='M79.4 31.5l-7.9 37h-9.6l7.9-37h9.6zm40.3 23.9l5-13.9 2.9 13.9h-7.9zm10.7 13.2h8.9l-7.7-37h-8.2c-1.8 0-3.4 1.1-4.1 2.7l-14.4 34.3H115l2-5.5h12.3l1.1 5.5zm-25-12.1c0-9.8-13.5-10.3-13.4-14.7 0-1.3 1.3-2.7 4.1-3.1 1.4-.2 5.2-.3 9.4 1.7l1.7-7.9c-2.3-.8-5.3-1.6-9-1.6-9.5 0-16.1 5-16.2 12.2-.1 5.3 4.8 8.3 8.4 10.1 3.7 1.8 5 3 5 4.6 0 2.5-3 3.6-5.7 3.6-4.8.1-7.6-1.3-9.8-2.3l-1.7 8.1c2.2 1 6.4 1.9 10.7 2 9.9-.1 16.5-5 16.5-12.7m-39.7-25l-15.5 37H40.1L32.4 39c-.5-1.8-.9-2.5-2.3-3.3-2.3-1.2-6.1-2.4-9.5-3.1l.2-1.1h16.3c2.1 0 3.9 1.4 4.4 3.8l4 21.4 10-25.2h10.2z'
								fill='url(#a)'
							/>
						</g>
					</switch>
				</svg>
				<svg
					xmlns='http://www.w3.org/2000/svg'
					viewBox='0 0 160 100'
					className='h-7'
				>
					<path
						d='M8 99c-3.8 0-7-3.2-7-7V8c0-3.8 3.2-7 7-7h144c3.8 0 7 3.2 7 7v84c0 3.8-3.2 7-7 7H8z'
						fillRule='evenodd'
						clipRule='evenodd'
					/>
					<path
						d='M152 2c3.3 0 6 2.7 6 6v84c0 3.3-2.7 6-6 6H8c-3.3 0-6-2.7-6-6V8c0-3.3 2.7-6 6-6h144zm0-2H8C3.6 0 0 3.6 0 8v84c0 4.4 3.6 8 8 8h144c4.4 0 8-3.6 8-8V8c0-4.4-3.6-8-8-8z'
						fill='#e6e6e6'
					/>
					<path
						d='M50.2 86.2v-4.8c0-1.8-1.2-3-3-3.1-1 0-2 .3-2.7 1.4-.5-.9-1.4-1.4-2.5-1.4-.8 0-1.6.2-2.2 1.1v-.9h-1.7v7.7h1.7V82c0-1.3.7-2 1.9-2 1.1 0 1.7.7 1.7 2v4.3H45V82c0-1.3.8-2 1.9-2 1.1 0 1.7.7 1.7 2v4.3h1.6zm25-7.7h-2.7v-2.3h-1.7v2.3h-1.6V80h1.6v3.5c0 1.8.7 2.8 2.7 2.8.7 0 1.6-.2 2.1-.6l-.5-1.4c-.5.3-1 .4-1.5.4-.8 0-1.1-.5-1.1-1.3v-3.5h2.7v-1.4zm14.2-.2c-1 0-1.6.5-2 1.1v-.9h-1.7v7.7h1.7v-4.3c0-1.3.5-2 1.6-2 .3 0 .7 0 1 .2l.5-1.6c-.2-.1-.7-.2-1.1-.2m-21.5.8c-.8-.5-1.9-.8-3.1-.8-1.9 0-3.2.9-3.2 2.5 0 1.3.9 2 2.6 2.3l.8.1c.9.1 1.3.4 1.3.8 0 .6-.6.9-1.7.9-1.2 0-2-.4-2.6-.8l-.8 1.3c.9.7 2.1 1 3.3 1 2.2 0 3.5-1 3.5-2.5 0-1.3-1-2.1-2.7-2.3l-.8-.1c-.7-.1-1.3-.2-1.3-.8 0-.6.5-.9 1.5-.9s1.9.4 2.4.7l.8-1.4zm44.8-.8c-1 0-1.6.5-2 1.1v-.9H109v7.7h1.7v-4.3c0-1.3.5-2 1.6-2 .3 0 .7 0 1 .2l.5-1.6c-.3-.1-.8-.2-1.1-.2m-21.6 4.1c0 2.3 1.6 4 4.1 4 1.2 0 1.9-.3 2.8-.9l-.8-1.3c-.6.5-1.3.7-2 .7-1.3 0-2.3-1-2.3-2.5s1-2.4 2.3-2.5c.7 0 1.4.2 2 .7l.8-1.3c-.8-.7-1.6-.9-2.8-.9-2.4-.1-4.1 1.6-4.1 4m15.7 0v-3.8h-1.7v.9c-.5-.7-1.3-1.1-2.4-1.1-2.2 0-3.8 1.7-3.8 4s1.7 4 3.8 4c1.1 0 1.9-.4 2.4-1.1v.9h1.7v-3.8zm-6.2 0c0-1.3.9-2.5 2.3-2.5s2.3 1.1 2.3 2.5-.9 2.5-2.3 2.5c-1.5-.1-2.3-1.2-2.3-2.5m-20.2-4.1c-2.2 0-3.8 1.6-3.8 4s1.6 4 3.9 4c1.2 0 2.2-.3 3.1-1.1l-.8-1.2c-.6.5-1.5.8-2.2.8-1.1 0-2.1-.5-2.3-1.9H84v-.6c0-2.3-1.4-4-3.6-4m0 1.5c1.1 0 1.8.7 1.9 1.9h-4c.2-1.1.9-1.9 2.1-1.9m41.9 2.6v-6.9h-1.7v4c-.5-.7-1.3-1.1-2.4-1.1-2.2 0-3.8 1.7-3.8 4s1.7 4 3.8 4c1.1 0 1.9-.4 2.4-1.1v.9h1.7v-3.8zm-6.2 0c0-1.3.9-2.5 2.3-2.5s2.3 1.1 2.3 2.5-.9 2.5-2.3 2.5c-1.5-.1-2.3-1.2-2.3-2.5m-56.5 0v-3.8H58v.9c-.5-.7-1.3-1.1-2.4-1.1-2.2 0-3.8 1.7-3.8 4s1.7 4 3.8 4c1.1 0 1.9-.4 2.4-1.1v.9h1.7v-3.8zm-6.2 0c0-1.3.9-2.5 2.3-2.5S58 81 58 82.4s-.9 2.5-2.3 2.5c-1.4-.1-2.3-1.2-2.3-2.5m71.7 2.7c.1 0 .2 0 .3.1.1 0 .2.1.2.2.1.1.1.2.2.2 0 .1.1.2.1.3 0 .1 0 .2-.1.3 0 .1-.1.2-.2.2-.1.1-.2.1-.2.2-.1 0-.2.1-.3.1-.1 0-.2 0-.3-.1-.1 0-.2-.1-.2-.2l-.2-.2c0-.1-.1-.2-.1-.3 0-.1 0-.2.1-.3 0-.1.1-.2.2-.2.1-.1.2-.1.2-.2.1-.1.2-.1.3-.1m0 1.4h.2c.1 0 .1-.1.2-.1.1-.1.1-.1.1-.2v-.4c0-.1-.1-.1-.1-.2-.1-.1-.1-.1-.2-.1h-.4c-.1 0-.1.1-.2.1-.1.1-.1.1-.1.2v.4c0 .1.1.1.1.2.1.1.1.1.2.1 0-.1.1 0 .2 0m0-1c.1 0 .2 0 .2.1 0 0 .1.1.1.2s0 .1-.1.1c0 0-.1.1-.2.1l.2.3h-.2l-.2-.3h-.1v.3h-.2v-.7h.5zm-.2.1v.2h.3v-.2h-.3z'
						fill='#fff'
					/>
					<path fill='#f16522' d='M67.3 20h25.3v45.5H67.3z' />
					<path
						d='M69 42.7c0-9.2 4.3-17.4 11-22.7-4.9-3.9-11.1-6.2-17.9-6.2-16 0-28.9 12.9-28.9 28.9 0 16 12.9 28.9 28.9 28.9 6.7 0 13-2.3 17.9-6.2-6.7-5.2-11-13.4-11-22.7'
						fill='#e41b24'
					/>
					<path
						d='M126.8 42.7c0 16-12.9 28.9-28.9 28.9-6.7 0-13-2.3-17.9-6.2 6.7-5.3 11-13.5 11-22.7S86.7 25.3 80 20c4.9-3.9 11.1-6.2 17.9-6.2 15.9 0 28.9 13 28.9 28.9m-2.8 18v-.9h.4v-.2h-1v.2h.4v.9h.2zm1.9 0v-1.1h-.3l-.3.8-.3-.8h-.3v1.1h.2v-.8l.3.7h.2l.3-.7v.8h.2z'
						fill='#f89e1c'
					/>
				</svg>
				<svg
					version='1.1'
					id='Layer_1'
					xmlns='http://www.w3.org/2000/svg'
					x='0'
					y='0'
					viewBox='0 0 160 100'
					xmlSpace='preserve'
					className='h-7'
				>
					<switch>
						<g>
							<path
								d='M8 99c-3.8 0-7-3.2-7-7V8c0-3.8 3.2-7 7-7h144c3.8 0 7 3.2 7 7v84c0 3.8-3.2 7-7 7H8z'
								fillRule='evenodd'
								clipRule='evenodd'
								fill='#fff'
							/>
							<path
								fill='#2671b9'
								d='M101.1 82.6l-3.9-4.7v4.7H86V76c-1.5.4-3 .6-5 .6h-1.7v6H63l-2.8-3.9-2.8 3.9H30.7V56.2h27l2.8 3.9 2.8-3.9h95.6V7.7c0-3.7-4-6.7-7.7-6.7H8.5C4.8 1 1.8 4 1.8 7.7v26l5.2-12h10.6l3 6.9v-6.9h12.9l1.9 5.8 1.9-5.8h57.3v2.6c2.2-1.7 5-2.6 8.2-2.6h17.4l3 6.9v-6.9h10.3l3.4 5.4v-5.4h11.6v26.4h-11l-3.2-5.1v5.1h-15.7l-1.5-3.9h-3.7l-1.5 3.9h-9.3c-2.6 0-5.6-.6-8-2.4v2.4H79.1l-3.7-4.7v4.7H16l-1.5-3.9h-3.7l-1.5 3.9H2v44.3C2 96 5 99 8.7 99h142.6c3.7 0 7.7-3 7.7-6.7V80.9c-1.7 1.1-4.7 1.5-6.8 1.5h-51.1v.2z'
							/>
							<path
								fill='#2671b9'
								d='M35 78.5h14.6v-3.9H39.3v-3.4h10.1v-3.9H39.3v-3.2h10.3v-3.9H35v18.3zm35.3 0L63 69.3l7.3-9h-5.4l-4.5 5.8-4.5-5.8h-5.6l7.3 9.2-7.3 9.2h5.4l4.5-5.8 4.5 5.8h5.6v-.2zm12.9-12.2c0 1.3-.9 2.1-2.4 2.1h-5.2v-4.5h5.2c1.6.2 2.4 1.1 2.4 2.4m4.3 0c0-3.6-2.4-6.2-6.2-6.2H71.2v18.2h4.5v-6h5.6c3.9.2 6.2-2.4 6.2-6m12.1 1.9h-5.2v-4.1h5.2c1.5 0 2.4.9 2.4 1.9-.2 1.1-.9 2.2-2.4 2.2m7.8 10.3l-6-6.6c3-.6 5-2.8 5-5.8 0-3.4-2.4-5.8-6.2-5.8h-9.9v18.2h4.5v-6.6H97l5.4 6.6h5zm1 0H123v-3.9h-10.3v-3.4h10.1v-3.9h-10.1v-3.2H123v-3.9h-14.6v18.3zm21.1-12.9c0-1.1.6-1.7 1.7-1.7h8l1.7-3.9h-9.5c-3.9 0-6.2 2.4-6.2 5.8s2.2 5.4 5.8 5.4h4.3c1.1 0 1.7.6 1.7 1.7s-.9 1.7-1.7 1.7h-9.5v3.9h9.3c3.9 0 6.2-2.4 6.2-5.8s-2.2-5.1-5.8-5.1h-4.3c-1-.2-1.7-.9-1.7-2m28.3-5.3l-9.5-.2c-3.9 0-6.2 2.4-6.2 5.8s2.2 5.4 5.8 5.4h4.3c1.1 0 1.7.6 1.7 1.7s-.9 1.7-1.7 1.7h-9.7v3.9h9.3c3 0 5.2-1.5 6-3.6.2-.6.4-1.3.4-2.1 0-.9-.2-1.5-.4-2.1-.9-1.9-2.6-3-5.4-3h-4.3c-1.1 0-1.7-.6-1.7-1.7s.6-1.7 1.7-1.7h8l1.7-4.1zM15.4 36.5H9.6l1.7-4.3 1.1-2.8 1.1 2.8 1.9 4.3zm3 7.5h5l-7.8-18.2H9.8L1.8 44h5l1.5-3.9h8.6l1.5 3.9zm24.3 0H47V25.8h-7.1l-3.7 10.5-.6 1.9-.6-1.9-3.7-10.5h-7.1V44h4.3V29.2l.9 2.8 4.1 12h4.1l4.3-12 .9-2.8V32c-.1 0-.1 12-.1 12zm7.8 0h14.6v-3.9H54.8v-3.4h10.1v-3.9H54.8v-3.2h10.3v-3.9H50.5V44zm27.3-10.3h-5.2v-4.1h5.2c1.5 0 2.4.9 2.4 1.9-.2 1.1-.8 2.2-2.4 2.2M85.6 44l-6-6.9c3-.6 5-2.8 5-5.8 0-3.4-2.4-5.8-6.2-5.8h-9.9v18.2H73v-6.6h2l5.4 6.6h5.2v.3zm1.1 0h4.5V25.6h-4.5V44zm16.1-14.4h4.7v-4.1h-5.2c-5.6 0-8.8 3.9-8.8 9v.5c0 5.4 3.2 9 8.4 9h.2l1.7-3.9h-1.3c-3 0-4.7-1.9-4.7-5.1v-.2c.3-3 2-5.2 5-5.2m15.5 6.9h-5.8l1.7-4.3 1.1-2.8 1.1 2.8 1.9 4.3zm3.1 7.5h5l-7.8-18.2h-5.8l-8 18.2h4.7l1.5-3.9h8.6l1.8 3.9zm23.2 0V25.8h-4.3v11.7l-1.3-1.9-6.5-9.9h-5.4V44h4.3V31.8l1.3 1.9 6.7 10.3h5.2z'
							/>
							<path
								d='M152 2c3.3 0 6 2.7 6 6v84c0 3.3-2.7 6-6 6H8c-3.3 0-6-2.7-6-6V8c0-3.3 2.7-6 6-6h144zm0-2H8C3.6 0 0 3.6 0 8v84c0 4.4 3.6 8 8 8h144c4.4 0 8-3.6 8-8V8c0-4.4-3.6-8-8-8z'
								fill='#e6e6e6'
							/>
						</g>
					</switch>
				</svg>
			</div>
			<div className='relative first-line:lg:mt-5 w-full border mt-4'>
				<input
					type='text'
					id='firstName'
					className='w-full h-16 pl-4 text-black form__item--input lg:rounded-r'
					required
				/>
				<label
					htmlFor='firstName'
					className='absolute text-neutral-400 my-auto top-5 lg:text-lg form__item--label'
				>
					First Name
				</label>
			</div>
			<div className='relative first-line:lg:mt-5 w-full border mt-4'>
				<input
					type='text'
					id='lastName'
					className='w-full h-16 pl-4 text-black form__item--input lg:rounded-r'
					required
				/>
				<label
					htmlFor='lastName'
					className='absolute text-neutral-400 my-auto top-5 lg:text-lg form__item--label'
				>
					Last Name
				</label>
			</div>
			<div className='relative first-line:lg:mt-5 w-full border mt-4'>
				<input
					type='text'
					id='cardNumber'
					className='w-full h-16 pl-4 text-black form__item--input lg:rounded-r'
					required
				/>
				<label
					htmlFor='cardNumber'
					className='absolute text-neutral-400 my-auto top-5 lg:text-lg form__item--label'
				>
					Card Number
				</label>
			</div>
			<div className='relative first-line:lg:mt-5 w-full border mt-4'>
				<input
					type='text'
					id='expirationDate'
					className='w-full h-16 pl-4 text-black form__item--input lg:rounded-r'
					required
				/>
				<label
					htmlFor='expirationDate'
					className='absolute text-neutral-400 my-auto top-5 lg:text-lg form__item--label'
				>
					Expiration date (MM/YY)
				</label>
			</div>
			<div className='relative first-line:lg:mt-5 w-full border mt-4'>
				<input
					type='text'
					id='securityCode'
					className='w-full h-16 pl-4 text-black form__item--input lg:rounded-r'
					required
				/>
				<label
					htmlFor='securityCode'
					className='absolute text-neutral-400 my-auto top-5 lg:text-lg form__item--label'
				>
					Security code (CVV)
				</label>
				<svg
					width='36'
					height='36'
					viewBox='0 0 36 36'
					xmlns='http://www.w3.org/2000/svg'
					className='absolute top-4 right-3'
				>
					<g fill='none' fillRule='evenodd'>
						<g>
							<circle
								stroke='#A9A6A6'
								strokeWidth='2'
								cx='18'
								cy='18'
								r='17'
							></circle>
							<path
								d='M17.051 21.094v-.54c0-.648.123-1.203.369-1.665.246-.462.741-.915 1.485-1.359a7.37 7.37 0 0 0 .981-.657c.222-.186.372-.366.45-.54.078-.174.117-.369.117-.585 0-.384-.177-.714-.531-.99-.354-.276-.831-.414-1.431-.414-.624 0-1.131.135-1.521.405-.39.27-.627.627-.711 1.071h-2.304a4.053 4.053 0 0 1 .738-1.845c.396-.546.924-.981 1.584-1.305.66-.324 1.44-.486 2.34-.486.852 0 1.596.153 2.232.459.636.306 1.134.726 1.494 1.26.36.534.54 1.143.54 1.827 0 .66-.177 1.227-.531 1.701-.354.474-.891.933-1.611 1.377-.42.252-.729.48-.927.684-.198.204-.33.399-.396.585a1.79 1.79 0 0 0-.099.603v.414h-2.268zm1.26 4.158c-.408 0-.762-.15-1.062-.45-.3-.3-.45-.654-.45-1.062 0-.408.15-.762.45-1.062.3-.3.654-.45 1.062-.45.408 0 .762.15 1.062.45.3.3.45.654.45 1.062 0 .408-.15.762-.45 1.062-.3.3-.654.45-1.062.45z'
								fill='#A9A6A6'
							></path>
						</g>
					</g>
				</svg>
			</div>
			<div className='flex justify-between items-center rounded bg-neutral-100 p-3 mt-4'>
				<div>
					<p className='font-semibold'>
						{money} <u>đ</u>/month
					</p>
					<p className='text-neutral-500 capitalize'>{selected} Plan</p>
				</div>
				<button
					className='text-blue-500 font-semibold'
					onClick={() => setNextPage(3)}
				>
					Change
				</button>
			</div>
			<p className='text-sm text-neutral-500'>
				Your payments will be processed internationally.
				<br />
				Additional bank fees mau apply
			</p>
			<p className='text-sm text-neutral-500 mt-8'>
				By checking the checkbox below, you agree to our{' '}
				<a
					href='https://help.netflix.com/legal/termsofuse'
					className='text-blue-500'
				>
					Terms of Use
				</a>
				,{' '}
				<a
					href='https://help.netflix.com/legal/privacy'
					className='text-blue-500'
				>
					Privacy Statement
				</a>{' '}
				and that you are over 18. Netflix will automatically continue your
				membership and charge the membership fee (currently 70,000 <u>đ</u>
				/month) to your payment method until you cancel. You may cancel at any
				time to avoid future charges.
			</p>
			<div className='mt-4'>
				<input
					type='checkbox'
					name='agree'
					id='agree'
					className='h-4 w-4 border border-gray-300 rounded-sm bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2'
				/>
				<label htmlFor='agree' className='ml-4 text-neutral-500'>
					I agree.
				</label>
			</div>
			<button className='w-full text-white bg-primary rounded mt-6 mb-16 py-3 text-3xl hover:bg-red-500'>
				Start Membership
			</button>
		</div>
	);
};

export default SixthBody;
